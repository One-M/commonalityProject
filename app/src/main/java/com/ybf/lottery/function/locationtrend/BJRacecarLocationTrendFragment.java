...
public class BJRacecarLocationTrendFragment extends BaseMvpFragment<LocationTrendPresenter> implements LocationTrendContract.IView, View.OnClickListener, ScrollChangeCallback {
    ...
    @BindView(R.id.dingwei_trend_ll)
    LinearLayout dingweill;
    @BindView(R.id.statistic_dingwei_trend_ll)
    LinearLayout statisticDingweill;
    ...
    @Override
    public LocationTrendPresenter initPresenter() {
        return new LocationTrendPresenter(this);
    }

    public static BJRacecarLocationTrendFragment newInstance(int trendType) {
        Bundle args = new Bundle();
        args.putInt(TREND_TYPE , trendType);
        BJRacecarLocationTrendFragment fragment = new BJRacecarLocationTrendFragment();
    ...
    private void initView(){
        ...
        statisticDingweill.setVisibility(View.VISIBLE);

        titleTxt.setText("定位走势");
    ...
    private void initData(String mPeriod){
        ...
        mPresenter.loadData(mPeriod , 1); // TODO=============================
    }

    @Override
    public void loginSuccess(LocationTrendBean locationTrendBean) {
        setStatus(STATUS_LOAD_SUCCESS);
        if (locationTrendBean == null) {
            return;
        }
        List<LocationTrendBean.ZsBean> zsData = locationTrendBean.getZs();
        //期号设置
    ...
    /**封装期数数据*/
    private List<String> getIssueNumData(List<LocationTrendBean.ZsBean> zsData){
        List<String> issueNum = new ArrayList<>();
        for (LocationTrendBean.ZsBean zs : zsData) {
    ...
    @Override
    public void loginFailed(LocationTrendBean locationTrendBean) {
